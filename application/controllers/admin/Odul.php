...
class Odul extends CI_Controller {
		...
		public function duzenle($id)
	{
		...
		$query=$this->db->query("SELECT * FROM belgelerimiz WHERE id=$id");
   ...
   $query=$this->db->query("SELECT * FROM belgelerimiz WHERE id=$id");
		...
		if ( !$this->upload->do_upload('resim'))//bu resim formdaki name="resim" den geliyor
        ...
        else
            {
               ...
               $this->load->model('Database_Model');
			   $this->Database_Model->update_data("belgelerimiz",$data,$id);
            }						
              	 ...
              	 $data=array(
                        ...
                        'belge_adi'=>$this->input->post('belge_adi'),
	 ...
	 public function ekle(){
		...
		$data["menu"]="Admin Paneli ";
	   	 $query=$this->db->query("SELECT * FROM belgelerimiz");
				$data["veriler"]=$query->result();
				$this->load->view('admin/odul_ekle',$data);
...
$query=$this->db->query("SELECT * FROM belgelerimiz");
  ...
  $config['upload_path']          = './uploads/';
			    ...
			    $upload_data=$this->upload->data();
                      ...
                      $data=array(
                       ...
                       'resim'=>$upload_data["file_name"],                    
                       	...
                       	$this->db->insert("belgelerimiz",$data);
			   $this->session->set_flashdata("mesaj","Yeni Ödül Eklendi !"); 
			   redirect(base_url()."admin/odul");
			}
		}
	...
	{
		 $this->db->query("DELETE FROM belgelerimiz WHERE id=$id");
